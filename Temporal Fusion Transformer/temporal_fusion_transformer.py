...
def generateTimeSeriesDataset(path, missingThreshold=0.1, columnToDelete=['wind_dir', 'unixtime'], start=None, end=(datetime.now().date())):
    ...
    training_cutoff = df["total_hours"].max() - max_prediction_length
...
def generateDataloaders(training, validation, batch_size=128):
...
def train(training, validation, batch_size=128):
    ...
    train_dataloader, val_dataloader = generateDataloaders(training, validation, batch_size)
...
def genPrediction(model_path, training, validation, batch_size=128):
...
def main():
    ...
    training, validation = generateTimeSeriesDataset(hourly_path, start=start, end=end)
    ...
    genPrediction(best_model_path, training, validation, 128)
